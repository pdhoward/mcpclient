...
export const useToolsFunctions = () => {
  ...
  const fetchSite = async ({ url }: { url: string }) => {
    ...
    try {
      ...
      toast.success(t('tools.fetchSite.toast') + " 📋", {
        description: t('tools.fetchSite.success'),
      })
    
      return {
        success: true,
        message: "Here is the scraped website content: " + JSON.stringify(scrapeResult.markdown) + "Let the user know you have successfully fetched the site content and if there are specific questions on the content that you can answer."
      };
